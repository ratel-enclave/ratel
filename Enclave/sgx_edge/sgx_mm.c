...
//#define DR_PATH "/source/SGX-DBI/libdynamorio.so"
#define DR_PATH "/home/sgx16/jhcui/sgx-dbi/test/SGX-DBI_SUCC/SGX-DBI-S1-Merge/libdynamorio.so"
...
static sgx_vm_area_t* _sgx_find_vma(byte *ext_start)
{
    ...
    while (ctuw && ll != &SGX_MM.in) {
        ...
        switch (ot) {
            ...
            case OVERLAP_SAME:
            case OVERLAP_MID: //missing a switch-case constant parameter in the function of _sgx_find_vma() to break out while loop.
                return vma;     //merged the change where a previously missing switch-case constant parameter in the function of _sgx_find_vma() was incuring an infinite loop in the while loop.
                break;          //fixed a unexpectedly missing switch-case constant parameter that was incurring an infinite loop.

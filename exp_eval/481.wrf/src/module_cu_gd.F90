...
!-------------------------------------------------------------
   SUBROUTINE GRELLDRV(DT,itimestep,DX,                         &
              rho_phy,RTHCUTEN,RQVCUTEN,RQCCUTEN,RQICUTEN,      &
              RAINCV,RTHFTEN,RQVFTEN,                           &
              RTHRATEN,RTHBLTEN,RQVBLTEN,                       &
              U,V,t_phy,W,q_phy,dz8w,p_phy,pi,                  &
              XLV,CP,G,r_v,                                     &
              P_QC,P_QI,P_QS,P_FIRST_SCALAR,STEPCU,             &
...
!                                                                      
     ...
     DO I=ITS,ITF
         ...
         T(I,K)=t_phy(i,k,j)
         q(I,K)=q_phy(i,k,j)
         omeg(I,K)= -g*rho_phy(i,k,j)*w(i,k,j)
         TN(I,K)=t(i,k)+RTHFTEN(i,k,j)*dt
         IF(TN(I,K).LT.200.)TN(I,K)=T(I,K)
         QO(I,K)=q(i,k)+RQVFTEN(i,k,j)*dt
...
!        RQVFTEN(i,k,j)=0.
     ...
     ENDDO
      do k=  kts+1,ktf-1
      DO I = its,itf
         if((p(i,1)-p(i,k)).gt.150.and.p(i,k).gt.300)then
      ...
      DO I = its,itf
        ...
        do k=kts,ktf
          print *,'2',tn(i,k),qo(i,k),pi(i,k,j),po(i,k)
        enddo
          print *,mconv(i)
        endif
      ENDDO
...
!
      CALL CUP_enss(outqc,j,AAEQ,T,Q,TER11,TN,QO,PO,PRET,P,OUTT,  &
           OUTQ,DT,PSUR,US,VS,tcrit,iens,                         &
      ...
      CALL neg_check(dt,q,outq,outt,outqc,pret,its,ite,kts,kte,itf,ktf)
...
!                if (rktop  < HTOP(i,j)) HTOP(i,j) = rktop
!                if (rkbcon > HBOT(i,j)) HBOT(i,j) = rkbcon
              endif
            ENDDO
            DO K=kts,ktf
            DO I=its,itf
        if(i.eq.ipr.and.j.eq.jpr)then
           print *,'3',i,k,outt(i,k),outq(i,k),outqc(i,k),pret(i)
        endif
...
!
  ...
  ! edt     = epsilon
     real,    dimension (its:ite) ::                                   &
       ...
       XMB,XPWAV,XPWEV,PWAV,PWEV,PWAVO,PWEVO,BU,BUO,cap_max
     integer,    dimension (its:ite) ::                                &
     ...
     real                                 ::                           &
      ...
      massfld,dh,cap_maxs,cap_max_increment
...
!
      ...
      call cup_env_clev(tn,qeso,qo,heo,heso,zo,po,qeso_cup,qo_cup, &
           ...
           ims,ime, jms,jme, kms,kme, &
           its,ite, jts,jte, kts,kte)
!     if(numchem.gt.1)then
!     call cup_env_clevt(trin,trcup,mix,kte,its,ite, &
!          numchem,ierr,              &
!          ids,ide, jds,jde, kds,kde, &
!          ims,ime, jms,jme, kms,kme, &
!          its,ite, jts,jte, kts,kte)
...
!            if(cd(i,k).gt.5.0*entr_rate)cd(i,k)=5.0*entr_rate
           enddo
        ENDIF
      ENDIF
      ENDDO
...
!     if(j.eq.jpr.and.iedt.eq.1)then
      if(j.eq.jpr)then
        i=ipr
        print *,'in 250 loop ',iedt,edt(ipr),ierr(ipr)
...
!
     ...
     integer                              ::                           &
        i,k,n,ncount
     real                                 ::                           &
        outtes,ddtes,dtt,dtq,dtqc,dtpw,tuning,prerate
     real,    dimension (its:ite)         ::                           &
       xfac1
     real,    dimension (its:ite)::                           &
       xmb_ske,xmb_ave,xmb_std,xmb_cur
     real,    dimension (its:ite)::                           &

...
{
    char buf[80];buf[0]=0; ev.gets(ev.prompt(), buf, 80); i=(short)std::atol(buf); return ev;
}
cEnvir& operator>> (cEnvir& ev, int& i)
{
    char buf[80];buf[0]=0; ev.gets(ev.prompt(), buf, 80); i=(int)std::atol(buf); return ev;
}
cEnvir& operator>> (cEnvir& ev, long& l)
{
    char buf[80];buf[0]=0; ev.gets(ev.prompt(), buf, 80); l=std::atol(buf); return ev;
}
cEnvir& operator>> (cEnvir& ev, unsigned short& i)
{
    char buf[80];buf[0]=0; ev.gets(ev.prompt(), buf, 80); i=(unsigned short)std::atol(buf); return ev;
}
cEnvir& operator>> (cEnvir& ev, unsigned int& i)
{
    char buf[80];buf[0]=0; ev.gets(ev.prompt(), buf, 80); i=(unsigned int)std::atol(buf); return ev;
}
cEnvir& operator>> (cEnvir& ev, unsigned long& l)
{
    char buf[80];buf[0]=0; ev.gets(ev.prompt(), buf, 80); l=(unsigned long)std::atol(buf); return ev;
}
cEnvir& operator>> (cEnvir& ev, double& d)
{
    char buf[80];buf[0]=0; ev.gets(ev.prompt(), buf, 80); d=std::atof(buf); return ev;
}

...
int critfunc_depth(const cKSplit& ks, cKSplit::Grid& g, int i, double *c)
{
     ...
     return g.cells[i] >= c[1]*std::pow(c[2],depth);
}
...
double cKSplit::random() const
{
   ...
   while (finish1 == 0)
   {
     ...
     xi += i * (rangemax - rangemin) / std::pow ((double)K, (double)cd);
   ...
   xa = xi + (rangemax - rangemin) / std::pow ((double)K, (double)cd);

#if defined(SPEC_CPU)
   x = std::rand () / (double)RAND_MAX * (xa - xi) + xi;
#else
...
double cKSplit::pdf (double x) const
{
   ...
   return realCellValue(lp,i) / std::pow ((double)K, (double)(dpth - cdpth));
}
